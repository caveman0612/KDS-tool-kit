...
var ignore_location = [
  '6c3ad14adb3f1f000da080c74b96198d',
  'c53a154adb3f1f000da080c74b96191a',
  'e83ad14adb3f1f000da080c74b961989',
];
...
function updateSecondFieldFilter(record) {
  ...
  if (
    secondFieldTables.length === 0 ||
    arrayUtil.contains(secondFieldTables, record.getRecordClassName())
  ) {
    for (var i = 0; i < secondFieldAliases.length; i++) {
      ...
      if (record.isValidField(someField)) {
        ...
        if (
          (!secondFieldCheckCurrentVal &&
            record.getValue(someField) !== secondFieldNewValue) ||
          (secondFieldCheckCurrentVal &&
            record.getValue(someField) === secondFieldCurrentVal)
        ) {
...
function taskDateFilter(record) {
  ...
  if (
    taskDateComparisonOperator !== -1 &&
    taskDateComparisonOperator !== 0 &&
    taskDateComparisonOperator !== 1
  ) {
    throw Error(
      'invalid taskDateComparisonOperator value: ' + taskDateComparisonOperator
    );
  }
  ...
  if (!taskDateField || !record.getLabel(taskDateField)) {
    throw Error(
      'missing taskDateField or invalid string provided: ' + taskDateField
    );
  }
  ...
  if (record) {
    ...
    if (!tableHasBaseTable(['task'], record.getTableName())) {
      ...
      record[taskDateField] &&
      tableHasBaseTable(['task'], record.getTableName()) &&
      pivotDate.compareTo(new GlideDateTime(record.getValue(taskDateField))) ===
        taskDateComparisonOperator
    ) {
      result = true;
    }
  }
...
function tableFilter(record) {
  for (var i = 0; i < ignoreTables.length; i++) {
    if (record.getTableName() == ignoreTables[i]) return false;
  }
...
function locationFilter(record) {
  for (var i = 0; i < ignore_location.length; i++) {
    if (
      record.getValue('u_location') == ignore_u_location[i] ||
      record.getValue('u_customer_location') == ignore_u_location[i]
    )
      return false;
  }
  return true;
...
function removeDuplicateLogObj(field) {
  if (dryrun) {
    ...
    for (var i = 0; i < output[field].length; i++) {
      ...
      var idAndFieldVal =
        output[field][i].record_sys_id + output[field][i].field;
...
function generateSmallLog() {
  ...
  for (var i = 0; i < output.updatesMade.length; i++) {
    ...
    else updatedTablesDict[output.updatesMade[i].table] += 1;
  }
}
